...
### Fixed
- Deployment, Job, and CronJob Pod specs now omit `serviceAccountName` when `serviceAccount.create=false` and no existing ServiceAccount name is configured

## [0.4.4] - 2026-09-22

### Added
- README instructions for connecting AI agents to the hosted Universal Helm Chart MCP knowledge service

### Fixed
- `serviceAccount.create=false` now prevents ServiceAccount creation instead of rendering and attempting to manage the namespace `default` ServiceAccount
...
### Added
- Optional `appProtocol` support for Service ports created by `ingressPlain` paths
- Values schema coverage and a tested gRPC Service example for `ingressPlain`

### Changed
- Updated the chart and configuration documentation with the new Service application protocol option
...
[0.4.5]: https://github.com/chaser100/u-helm-chart/releases/tag/v0.4.5
[0.4.4]: https://github.com/chaser100/u-helm-chart/releases/tag/v0.4.4
